...
```tsx
import { Router, parseSearchWith, stringifySearchWith } from "@tanstack/solid-router";
...
```tsx
import { createRootRouteWithContext, createRouter } from "@tanstack/solid-router";
...
export const Route = createRootRoute({
  component: () => {
    ...
    return (
      <div>
        <For each={matches()}>{(match) => <div>{match.staticData.customData}</div>}</For>
      </div>
    );
  },
});
...
export default defineConfig({
  plugins: [tanstackRouter({ virtualRouteConfig: routes, target: "react" }), react()],
});
```
...
export default defineConfig({
  plugins: [tanstackRouter({ virtualRouteConfig: routes, target: "solid" }), solid()],
});
```
...
const ctx = await esbuild.context({
  ...
  target: ["esnext"],
  sourcemap: true,
  plugins: [solidPlugin(), tanstackRouter({ target: "solid", autoCodeSplitting: true })],
});
...
function App() {
  return (
    <>
      <RouterProvider router={router} />
      <TanStackRouterDevtoolsPanel router={router} style={styles} class={className} />
    </>
  );
}
```
